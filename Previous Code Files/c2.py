...
def create_severity_breakdown(results):
    ...
    for check in results:
        if not check['passed'] and check['severity'] >= 4:
            if check['severity'] >= 8:
...
def generate_recruiter_insights(results):
    ...
    if deal_breakers:
        insights.append({
            ...
            "items": [f"{check['check_name']} ({check['category']}): {check['explanation']}" for check in deal_breakers]
        })
    ...
    if red_flags:
        insights.append({
            ...
            "items": [f"{check['check_name']} ({check['category']}): {check['explanation']}" for check in red_flags]
        })
                        ...
                        with tab1:
                            for check in results:
                                if check['passed']:
                                    ...
                                    (Section: {check['category']}, Severity: {check['severity']}/10)</span>
