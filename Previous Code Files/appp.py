...
def build_analysis_prompt(resume_text: str) -> str:
    ...
    prompt = f"""
You are an expert resume reviewer tasked with identifying anomalies based on a predefined list of 12 checks. For each check, you will output a single bullet point using the following format:

- ✅ Short OK confirmation.
- ❌ **Very short explanation of the problem.**

Your output MUST contain exactly 12 bullet points, one for each check in the provided list. Be concise and adhere strictly to the formatting.
