...
Return the results as a JSON array of 13 objects, one for each check, following the format specified above.
'''
...
def create_resume_health_gauge(overall_score):
    ...
    # Set width to properly display scale from 0 to 100
    fig.update_layout(height=250, width=400, margin=dict(l=20, r=20, t=50, b=20))
    return fig
...
def create_severity_breakdown(results):
    ...
    # Filter out severity levels with zero counts
    filtered_levels = {k: v for k, v in severity_levels.items() if v > 0}
    
    # Create horizontal bar chart with counts displayed directly
    fig = go.Figure()
    colors = ['red', 'orange', 'yellow', 'green']
    
    for i, (level, count) in enumerate(filtered_levels.items()):
        fig.add_trace(go.Bar(
            y=[f"{level} - {count}"],  # Display count directly in the label
            x=[count],
            orientation='h',
...
def main():
    ...
    # Custom CSS - Removed styling for metric-card class as requested
    st.markdown("""
    <style>
    .main {
    ...
    # App Header - Single title display
            ...
            with st.spinner("Analyzing resume... This may take a moment."):
                ...
                if resume_text:
                    ...
                    if not analysis["parsed"]:
                        ...
                        # Top metrics - Removed container divs
                        col1, col2, col3 = st.columns(3)
                        
                        with col1:
                            st.markdown("<div class=\"section-title\">Resume Health Score</div>", unsafe_allow_html=True)
                        ...
                        with col3:
                            st.markdown("<div class=\"section-title\">Anomaly Severity</div>", unsafe_allow_html=True)
                        ...
                        if not recruiter_insights:
                            ...
                            for insight in recruiter_insights:
                                # Skip "Resume Strengths" section as requested
                                if insight['title'] == "Resume Strengths":
                                    continue
                        ...
                        with tab1:
                            for check in results:
                                if check['passed']:
                                    ...
                                    # Removed the "Section:" field as requested
                                    st.markdown(f"""
                        ...
                        with tab2:
                            ...
                            if not failed_checks:
                                ...
                                if critical:
                                    ...
                                    for i, check in enumerate(critical):
                                        # Removed the "Section:" field as requested
                                        st.markdown(f"""
                                ...
                                # Only add separator if both critical and moderate issues exist
                                if critical and moderate:
                                    st.markdown("---")
                                
                                if moderate:
                                    st.markdown("#### Moderate Issues")
                                    for i, check in enumerate(moderate):
                                        # Removed the "Section:" field as requested
                                        st.markdown(f"""
                        ...
                        # Export options - removed JSON download button
